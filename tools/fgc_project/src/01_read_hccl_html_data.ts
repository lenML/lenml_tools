/**
 * 这个脚本读取 ./hccl_html_data 下面的文件夹
 * 下面有类似 ./hccl_html_data/西游记 这样的文件夹，其中每个 txt 文件为一章节
 * 文件名类似 `001 第一回 靈根育孕源流出 心性修持大道生.txt` 其中 001 为 index,后面的为标题
 * 这样的文件夹结构
 *
 * 下面的脚本用于解析这样的数据，并储存为一个 json 对象
 * 注意有多个文件夹需要读取，都是类似的格式 `./hccl_html_data/三国演义` `./hccl_html_data/红楼梦`
 */
import fs from "fs";
...
(async () => {
  ...
  fs.writeFileSync("books2.json", JSON.stringify(books, null, 2));
  ...
  fs.writeFileSync("segments2.json", JSON.stringify(segments, null, 2));
